...
namespace DataOutBase
{
  ...
  struct CFFlags : public OutputFlagsBase<CFFlags>
  {
     ...
     * Attributes are only written when the file is first created, not
...
public:
   ...
   * DataOutBase::CFFlags for options to control the output. Currently
   * only supports 2D meshes (codimension 0) that are either
   * only quads or only triangles, no mixed meshes.
   * Below is an example of how to use this function with the DataOutFilter:
   *
   * @code
   * // Filter the data and store it in data_filter
   * DataOutBase::DataOutFilterFlags flags(true, true);
